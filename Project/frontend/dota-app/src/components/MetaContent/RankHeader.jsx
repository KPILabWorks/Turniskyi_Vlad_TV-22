...
function RankHeader ({rank, setRank, gamemode, setGamemode}) {
    return(
        <div className='flex justify-center items-stretch space-x-4 mt-3 mx-auto'>
        {/* GAME MODE */}
        <div className="flex flex-col mx-2 p-4 rounded">
            <p className='text-2xl mb-2 font-semibold'>GAME MODE</p>
            <ul className="flex flex-1 justify-center content-center gap-2 border border-yellow-950 border-2 rounded-sm p-1 bg-yellow-800">
                <li className='flex content-center gap-2'>
                    <a className={"py-1 px-2 content-center rounded-sm text-white text-xl my-2 "
                        + (gamemode === GameMode.ALL_PICK ? "bg-yellow-950 " : "hover:bg-yellow-900 hover:cursor-pointer")
                    }
                    onClick={() => setGamemode(GameMode.ALL_PICK)}
                    >
                        All Pick
                    </a>
                </li>
                <li className='flex content-center gap-2'>
                    <a className={"py-1 px-3 content-center rounded-sm text-white text-xl my-2 "
                        + (gamemode === GameMode.TURBO ? "bg-yellow-950 " : "hover:bg-yellow-900 hover:cursor-pointer")
                    }
                    onClick={() => setGamemode(GameMode.TURBO)}
                    >
                        Turbo
                    </a>
                </li>
        ...
        <div className="flex  flex-col  p-4 ">
            ...
            <ul className="flex content-center border border-yellow-950 border-2 rounded-sm p-1 bg-yellow-800"> 
                <li className='flex content-center gap-2'>
                    <a className={'content-center text-2xl m-1 px-3 p-2 rounded '
                    + (rank === Rank.ALL ? "bg-yellow-950 " : "hover:bg-yellow-900 hover:cursor-pointer")
                    }
                    onClick={() => setRank(Rank.ALL)}
                    >
                        All
                    </a>
                </li>
